...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingIps = new Set(existingProxies?.map((p) => p.ip) || []);
    ...
    const skippedCount = validProxies.length - newProxies.length;

    console.log(
      `✅ プロキシバルクインサート成功: 新規登録=${newProxies.length}個, スキップ=${skippedCount}個`
    );
